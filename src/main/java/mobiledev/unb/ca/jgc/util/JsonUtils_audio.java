...
public class JsonUtils_audio {
    ...
    private ArrayList<AudioInfo> hostCitiesArray;

    // Initializer to read our data source (JSON file) into an array of host city objects
    public JsonUtils_audio(Context context) {
        processJSON(context);
    }
    ...
    private void processJSON(Context context) {
        ...
        try {
            ...
            for (int i=0; i < jsonArray.length(); i++) {
                ...
                AudioInfo city = new AudioInfo.Builder(elementObject.getString(KEY_YEAR),
